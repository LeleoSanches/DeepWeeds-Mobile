...
# 1 - Alterar coluna Label para STR - Mudar o pipeline.
# 2 - Mudar salvamento dos modelos de .hdf5 para .Keras - (Tensorflow 2 +)
# - Alterar o parâmetro do optimizador em model.compile: Adam(lr=0.0001) -> Adam(learning_rate=)
# - Substituir função Fit_generator -> Desatualizado
# 3 - Implementar MobileNetV3 no Pipeline
...
IMG_DIRECTORY = "./images2/images/"
#IMG_DIRECTORY = "/home/leo/Downloads/images/"
...
def cross_validate(model_name):
    ...
    for k in range(FOLDS):
        ...
        # Arrumando Erro de Dtype no Flow_Dataframe
        train_dataframe['Label'] = train_dataframe["Label"].astype(str)
        # Remove espaços no início/fim
        train_dataframe['Label'] = train_dataframe['Label'].str.strip()
        print(IMG_DIRECTORY + train_dataframe['Filename'][0])
        ...
        train_data_generator = train_data_generator.flow_from_dataframe(
            ...
            save_format = 'jpeg')
        print(train_data_generator.samples)
        ...
        model_checkpoint = ModelCheckpoint(output_directory + "lastbest-0.keras", verbose=1, save_best_only=True)
        ...
        model.compile(loss='binary_crossentropy', optimizer=Adam(learning_rate=INITIAL_LR), metrics=['categorical_accuracy'])
        ...
        while global_epoch < MAX_EPOCH:
            ...
            if early_stopping.stopped_epoch == 0:
                ...
                restarts = restarts + 1
                model.compile(loss='binary_crossentropy', optimizer=Adam(learning_rate=INITIAL_LR / 2 ** restarts),
        ...
        model = load_model(
            output_directory + "lastbest-{}.keras".format(last_best_losses.index(min(last_best_losses))))
